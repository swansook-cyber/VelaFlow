...
```text
project_data/music/<project>/creative_direction.json
```

V7.8.6a also creates a lightweight song structure plan:

```text
project_data/music/<project>/song_structure_plan.json
```

The plan includes section order, energy curve, emotional arc, hook placement,
and notes for lyric generation. It is a producer workflow guide, not a DAW,
chord generator, or MIDI system.
...
and song structure plan as extra context while keeping:
