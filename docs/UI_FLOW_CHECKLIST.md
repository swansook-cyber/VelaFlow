...
- App launches on `http://localhost:8502/`.
...
| Save direction | PASS | Writes `project_data/music/<project>/creative_direction.json` with legacy fallback for `project_data/projects/` |
...
| Page / Flow | Status | Notes |
|---|---|---|
| Creator Wizard | PASS | Adds Song Structure selection and previews section energy / emotional arc |
| Song Studio | PASS | Adds Song Structure Intelligence expander and uses plans in hook/lyrics prompts |
| Save Lyrics | PASS | Keeps Thai lyrics and English-only parentheses tag normalization |
| Suno Export | PASS | Exports `song_structure_plan.json` and `song_structure_plan.md` with Suno files |
| Template Loader | PASS | Preserved under `Advanced: Apply Project Template` |
| Navigation | PASS | Existing sidebar and pending navigation behavior unchanged |
