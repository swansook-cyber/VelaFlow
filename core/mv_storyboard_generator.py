...
from core.visual_engine import build_scene_flow, build_visual_prompt
...
def _metadata_from_song(song: Dict[str, Any], project: Dict[str, Any] | None = None) -> Dict[str, str]:
    project = project or {}
    artist_preset_data = song.get("artist_preset_data") if isinstance(song.get("artist_preset_data"), dict) else {}
    ...
    mv_identity = artist_preset_data.get("mv_identity", {}) if isinstance(artist_preset_data.get("mv_identity"), dict) else {}
    return {
        "song_title": _song_value(song, "song_title", "title") or _clean_text(project.get("title")) or "Untitled Song",
        "genre": _song_value(song, "genre") or _clean_text(artist_preset_data.get("genre")) or _clean_text(music_preset_data.get("genre")) or "Modern Pop / Pop Rock",
        "mood": _song_value(song, "mood") or _clean_text(artist_preset_data.get("mood")) or _clean_text(music_preset_data.get("mood")) or _clean_text((project.get("creative_direction") or {}).get("mood")) or "emotional, cinematic",
        ...
        "artist_visual_mood": _clean_text(mv_identity.get("visual_mood")),
        "artist_camera_language": ", ".join(mv_identity.get("camera_language", []) or []) if isinstance(mv_identity.get("camera_language"), list) else _clean_text(mv_identity.get("camera_language")),
    }
...
def generate_mv_storyboard(
    ...
    try:
        ...
        visual_config = normalize_visual_settings(visual_settings)
        metadata["visual_engine"] = visual_config
        metadata["scene_structure"] = build_scene_flow("music_mv", visual_config)
        lyrics = (
        ...
        artist_visual_mood = metadata.get("artist_visual_mood", "")
        artist_camera_language = metadata.get("artist_camera_language", "")
        ...
        for index in range(count):
            ...
            camera = visual_config["camera_description"] if visual_settings else _pick(CAMERA_BANK, index, len(preset))
            lighting = visual_config["lighting_description"] if visual_settings else _pick(LIGHTING_BANK, index, len(genre))
            ...
            if artist_visual_mood:
                scene_mood = f"{scene_mood}, {artist_visual_mood}"
            ...
            visual_prompt = build_visual_prompt(
                ...
                subject=visual,
                context=f"lyric moment: {lyric[:80]}, {scene_mood}, {genre}, artist camera language: {artist_camera_language}",
                visual_settings=visual_config,
...
def storyboard_to_text(storyboard: List[Dict[str, Any]], metadata: Dict[str, Any] | None = None) -> str:
    ...
    lines = [
        ...
        f"Camera Preset: {((metadata.get('visual_engine') or {}).get('camera_preset') or '-')}",
        f"Lighting Preset: {((metadata.get('visual_engine') or {}).get('lighting_preset') or '-')}",
        f"Motion Preset: {((metadata.get('visual_engine') or {}).get('motion_preset') or '-')}",
        f"Visual Mood: {((metadata.get('visual_engine') or {}).get('visual_mood') or '-')}",
...
def export_mv_storyboard(
    ...
    try:
        project_dir = Path(base_dir) / safe_name(project_name or "project") if base_dir else resolve_project_folder(project_name or "project", "mv")
