...
PROJECT_ROOT = workflow_project_root("song")
...
def resolve_song_generation_context(
    *,
    ...
    return {
        ...
        "production_intelligence_selected_profile": production_profile["production_intelligence_selected_profile"],
        "production_intelligence": dict(production_profile["production_intelligence"]),
...
def generate_hook_candidates(idea: str = "", artist_preset: Dict[str, Any] | None = None, seed: str | None = None) -> List[Dict[str, Any]]:
    ...
    preset_id = str(preset.get("artist_id") or "")
    category = str(preset.get("category") or "")
    mood = str(preset.get("mood") or "")
    if "viral" in preset_id:
        hooks += ["ท่อนแรกต้องหยุดฟัง", "อย่าเพิ่งเลื่อน ถ้าใจยังเจ็บ", "ประโยคนี้แทนใจเกินไป"]
    if "dark" in preset_id or "night" in preset_id or "lonely" in mood:
        hooks += ["คืนยิ่งมืด ใจยิ่งดัง", "ไฟเมืองยังสว่าง แต่ใจฉันดับ", "เหงาจนจำทางกลับไม่ได้"]
    if "motivational" in preset_id:
        hooks += ["ล้มได้ แต่อย่าหยุดเดิน", "พรุ่งนี้ยังรอเราอยู่", "เจ็บวันนี้ เพื่อโตพรุ่งนี้"]
    if "cozy" in preset_id or "acoustic" in preset_id:
        hooks += ["พักใจไว้ตรงนี้ก่อน", "แค่มีเพลงนี้ก็พอ", "ค่อย ๆ หายใจไปด้วยกัน"]
    if preset_id == "vela_moon" or category == "Vela Moon Signature":
...
"""
    ...
    try:
        ...
        return {
            "ok": True,
            "message": "Using offline fallback hooks",
            "data": {
                "hooks": hooks,
                "offline": True,
                "seed": seed_value,
