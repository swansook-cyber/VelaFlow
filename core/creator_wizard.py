...
def save_creative_direction(project_name: str, direction: Dict[str, Any], base_dir: str | Path | None = None) -> Dict[str, Any]:
    try:
        folder = Path(base_dir) / safe_name(project_name or suggest_project_name(direction.get("topic", "New Song"))) if base_dir else resolve_project_folder(project_name or suggest_project_name(direction.get("topic", "New Song")), "music_pipeline")
...
def load_creative_direction(project_name: str, base_dir: str | Path | None = None) -> Dict[str, Any]:
    try:
        path = (Path(base_dir) / safe_name(project_name or "project") if base_dir else resolve_project_folder(project_name or "project", "music_pipeline")) / "creative_direction.json"
        if not path.exists():
