...
"""
        ...
        assert_true("st.title(title)" in page_header_source and "VelaFlow Beta" not in page_header_source and "Current project:" not in page_header_source and "st.image" not in page_header_source, "workspace header must render one workspace title without repeated product branding")
        assert_true('st.markdown("### VelaFlow")' in sidebar_source and "AI Music Production" in sidebar_source and 'st.radio(' in sidebar_source and sidebar_source.count('"Navigation"') == 1 and 'label_visibility="collapsed"' not in sidebar_source and 'key="selected_page"' in sidebar_source and "st.image" not in sidebar_source, "sidebar branding/navigation should be compact, text-only, and contain one Navigation heading")
        ...
        assert_true(home_source.count('st.title("VelaFlow")') == 1 and 'st.caption("AI Music Production")' in home_source and all(label in home_source for label in ["Song Studio", "Remaster", "Cut Audio", "Join Audio", "Visual Studio", "Release Pack", "Settings"]), "simple Home actions missing")
