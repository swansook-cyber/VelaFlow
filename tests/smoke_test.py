...
"""
        ...
        sidebar_source = main_source[main_source.find("with st.sidebar:"):main_source.find("if selected_mode != st.session_state.get")]
        assert_true('st.markdown(f"### VelaFlow Beta {APP_VERSION}")' in page_header_source and page_header_source.count("VelaFlow Beta") == 1 and "st.image" not in page_header_source and "logo_col" not in page_header_source and "brand_col" not in page_header_source, "main header should be text-only with no visible VelaFlow logo image")
        assert_true('st.markdown("### VelaFlow")' in sidebar_source and "AI Music Production" in sidebar_source and "st.image" not in sidebar_source and "brand_logo_col" not in sidebar_source and "brand_text_col" not in sidebar_source, "sidebar branding should be text-only")
