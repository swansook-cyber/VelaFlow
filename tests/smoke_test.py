...
def main():
    ...
    assert_true("@media (max-width: 768px)" in css and "min-height: 2.75rem" in css and "padding-top: max(4.5rem" in css and "env(safe-area-inset-top) + 3.75rem" in css and "padding-left: 0.9rem" in css and "gap: 0.3rem" in css and "st-key-velaflow_local_api_read" in css, "safe compact mobile UI styles failed")
    assert_true(all(token in css for token in ["--vf-primary: #4257e8", "--vf-radius-card: 16px", ".vf-home-header", ".vf-welcome-card", ".vf-page-header", ".vf-tool-icon", ".vf-workspace-card-title", ".vf-workspace-section", ".vf-sidebar-logo", ".vf-about-brand", ".st-key-vf_home_tools", ".st-key-vf_song_form", ".st-key-vf_remaster_source_card", ".st-key-vf_audio_mode_card"]), "premium mobile design system is incomplete")
    assert_true('div[role="radiogroup"] > label:has(input:checked)' in css and 'label > div:first-child' in css and "display: none" in css, "sidebar navigation does not expose an app-like active state or still shows radio circles")
    ...
    assert_true(SIMPLE_NAVIGATION_PAGES == ["Home", "Song Studio", "Remaster Studio", "Audio Editor", "Visual Studio", "Release Pack", "Settings", "About"] and len(SIMPLE_NAVIGATION_PAGES) == len(set(SIMPLE_NAVIGATION_PAGES)), "simple navigation order or uniqueness failed")
    assert_true(PAGE_LABELS.get("About") == "About", "About navigation label missing")
...
"""
        ...
        about_source = main_source[main_source.find("def _render_about"):main_source.find("def _render_creator_dashboard")]
        settings_controls_source = main_source[main_source.find("def _render_settings_system_controls"):main_source.find("def _render_settings_about")]
        settings_about_source = main_source[main_source.find("def _render_settings_about"):main_source.find("def _render_agent_workspace_panel")]
        settings_page_source = main_source[main_source.find('elif page in {"Settings", "AI Settings"}'):main_source.find('elif page == "Artist Preset Manager"')]
        pipeline_status_source = main_source[main_source.find("def _render_music_pipeline_status"):main_source.find("def _render_audio_editor_legacy")]
        assert_true("vf-page-header" in page_header_source and "<h1>" in page_header_source and "st.title(title)" not in page_header_source and "VelaFlow Beta" not in page_header_source and "Current project:" not in page_header_source and "st.image" not in page_header_source, "workspace header must render one compact workspace title without repeated product branding")
        assert_true("vf-sidebar-brand" in sidebar_source and "vf-sidebar-logo" in sidebar_source and "VELAFLOW_STATIC_ICON_192" in sidebar_source and "AI Music Production" in sidebar_source and 'st.radio(' in sidebar_source and sidebar_source.count('"Navigation"') == 1 and 'format_func=_premium_navigation_label' in sidebar_source and 'key="selected_page"' in sidebar_source and "st.image" not in sidebar_source, "sidebar logo/navigation should be compact, app-like, and contain one Navigation heading")
        assert_true(all(text not in sidebar_source for text in ["Creator Navigation", "Workflow Mode", "Advanced / Developer Mode", "License Status", "Provider Status", "Project Management", "Founding Member", "Closed Beta", "Section"]), "sidebar still contains duplicated navigation or admin controls")
        assert_true(home_source.count("<h1>VelaFlow</h1>") == 1 and "AI Music Production" in home_source and "vf-welcome-card" in home_source and "vf_home_tools" in home_source and all(label in home_source for label in ["Song Studio", "Remaster", "Cut Audio", "Join Audio", "Visual Studio", "Release Pack", "Settings"]), "premium Home actions or hierarchy missing")
        assert_true(all(text not in home_source for text in ["Closed Beta", "Founding Member", "License", "Provider", "Project Management", "Quick Start", "Workflow Mode"]), "Home contains beta/admin/debug clutter")
        assert_true(all(text in about_source for text in ["About VelaFlow", "AI Music Production by VelaLab", "VelaFlow is an AI-assisted music production workspace", "www.velalab.net", "Visit VelaLab", "https://www.velalab.net", "APP_VERSION", "BUILD_VERSION", "VELAFLOW_STATIC_ICON_192"]) and 'st.link_button(' in about_source, "compact VelaFlow/VelaLab About page or external link missing")
        assert_true('elif page == "About":' in main_source and "_render_about()" in main_source and '"About": "ⓘ"' in main_source, "About navigation route or icon missing")
        assert_true(all(text in settings_controls_source for text in ["Project Management", "App Preferences", "Advanced / Developer Mode", "Workflow Mode"]), "project or app preference groups missing from Settings")
        ...
        assert_true(all(label in simple_song_source for label in ["Project / Song Title", "Artist", "Song Idea / Story", "Genre", "Mood", "Vocal", "Generate Title", "Generate Lyrics", "Lyrics Check", "Export to Suno", 'expander("Advanced"']), "simplified Song Studio controls missing")
        assert_true('st.container(border=True, key="vf_song_form")' in simple_song_source and 'song_form.button("Generate Lyrics"' in simple_song_source and 'song_form.expander("Advanced", expanded=False)' in simple_song_source and "vf-workspace-card-title" in simple_song_source and "vf-title-row-marker" in simple_song_source and "Creative direction" in simple_song_source, "Song Studio creation panel or primary CTA styling hook missing")
        ...
        access_password_source = main_source[main_source.find("def _configured_access_password"):main_source.find("def _render_access_gate")]
        ...
        assert_true("st.image(str(VELAFLOW_ICON_512)" not in main_source and "st.image(str(VELAFLOW_ICON_192)" not in main_source and "st.image(str(VELAFLOW_APPLE_TOUCH_ICON)" not in main_source, "workspace branding should avoid duplicate Streamlit image blocks")
        ...
        assert_true("_cached_audio_probe" in main_source and "audio_ffprobe_cache" in main_source and "_waveform_source_signature" in main_source and "max_cache_bytes = 64 * 1024 * 1024" in main_source, "audio probe/waveform/preview cache reuse wiring missing")
        ...
        assert_true("Preset Selection" in remaster_ui_slice and "Auto Recommended" in remaster_ui_slice and "Analyze Source" in remaster_ui_slice and "Recommended by VelaFlow" in remaster_ui_slice and 'preset_card.selectbox("Preset", REMASTER_STYLES' in remaster_ui_slice and "_render_custom_remaster_controls" in remaster_ui_slice and "Custom processing controls are reserved" not in remaster_ui_slice and "Use Recommended Preset" not in remaster_ui_slice and "Choose Manually" not in remaster_ui_slice and "Advanced Settings" not in remaster_ui_slice, "simple Auto/Manual Remaster preset UI missing")
        assert_true(all(token in remaster_ui_slice for token in ['key="vf_remaster_source_card"', 'key="vf_remaster_preset_card"', 'key="vf_remaster_process_card"', "Source audio", "Mastering preset", "Process master"]) and 'source_card.file_uploader(' in remaster_ui_slice and 'preset_card.radio(' in remaster_ui_slice and 'process_card.button("Process Audio"' in remaster_ui_slice, "Remaster premium card grouping or existing control wiring missing")
        ...
        regions_vendor = waveform_vendor_dir / "regions-7.12.11.min.js"
        wavesurfer_license = waveform_vendor_dir / "WAVESURFER_LICENSE.txt"
        ...
        assert_true('mode_card.radio("Mode", ["Cut", "Join"]' in active_editor_slice and "_render_audio_joiner" in active_editor_slice and "_render_audio_cutter" in active_editor_slice, "Audio Editor Cut/Join mode switch missing")
        assert_true(all(token in active_editor_slice for token in ['key="vf_audio_mode_card"', "Choose a workflow"]) and all(token in cutter_slice for token in ['key="vf_audio_source_card"', 'key="vf_audio_cut_workspace"', 'key="vf_audio_export_card"', "Audio source", "Waveform editor", "Export selection"]) and all(token in joiner_slice for token in ['key="vf_audio_join_source_card"', "Audio files", "Track arrangement"]), "Audio Editor premium workflow grouping missing")
        ...
        assert_true("+ Add Tracks" in joiner_slice and "Track arrangement" in joiner_slice and "Crossfade" in joiner_slice and "Play Transition" in joiner_slice and "Join & Export" in joiner_slice and "accept_multiple_files=True" in joiner_slice and "audio_joiner" in joiner_slice, "Audio Joiner V2 workflow missing")
        ...
        continuous_update_source = waveform_component_source[waveform_component_source.find('regionsPlugin.on("region-update"'):waveform_component_source.find('regionsPlugin.on("region-updated"')]
        ...
        assert_true("AI Creative Pack Generator" in main_source and "Generate Song" in main_source and "Generate Final Release Pack" in main_source and "No Render" in main_source and "Create a focused, copy-ready music release package." in main_source and "แนวเพลงหลัก" in main_source and "ชื่อศิลปิน" in main_source, "creative pack generator UI missing")
        ...
        assert_true('f"{provider_label} API Key"' in main_source and "Gemini status:" in main_source and "Key source:" in main_source and "Gemini configure() result:" in main_source and "Gemini client initialization result:" in main_source and "Test Gemini Connection" in main_source, "Gemini settings diagnostics UI missing")
        assert_true("settings_quick_gemini_api_key" not in main_source and 'st.expander("Advanced / Diagnostics", expanded=False)' in main_source and 'f"{provider_label} API Key"' in main_source, "Settings should use one provider key input and collapse runtime diagnostics")
        ...
        assert_true('MENU_GROUPS = {"NAVIGATION": list(SIMPLE_NAVIGATION_PAGES)}' in main_source and 'SIMPLE_PAGE_ALIASES' in main_source and '_simple_page_name' in main_source, "single navigation model wiring missing")
        ...
        assert_true("VelaFlow Agent Studio" not in SIMPLE_NAVIGATION_PAGES and "Audio Editor" in SIMPLE_NAVIGATION_PAGES and "Settings" in SIMPLE_NAVIGATION_PAGES and SIMPLE_NAVIGATION_PAGES[-1] == "About", "advanced destinations leaked into compact navigation or About is misplaced")
