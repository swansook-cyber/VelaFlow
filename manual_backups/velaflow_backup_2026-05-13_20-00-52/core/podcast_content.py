...
def export_podcast_content(project_name: str, package: Dict[str, Any], base_dir: str | Path | None = None) -> Dict[str, Any]:
    try:
        root = Path(base_dir) if base_dir else Path(__file__).resolve().parents[1] / "project_data" / "projects"
        folder = root / safe_name(project_name or package.get("episode_title") or "podcast_episode")
