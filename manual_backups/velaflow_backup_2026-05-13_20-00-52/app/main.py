...
def _active_text_credentials() -> tuple[str, str, str]:
    ...
    if provider == "openai":
        return provider, settings.openai_api_key, settings.openai_text_model
    return provider, settings.gemini_api_key, settings.gemini_model
...
if page == "Dashboard":
    ...
    if st.button("Generate Seller Content", type="primary", use_container_width=True):
        product_image_meta = (project.get("seller_studio", {}) or {}).get("product_image", {}) or {}
        result = generate_seller_content(product_name, product_category, target_audience, key_points, tone_style, product_image=product_image_meta, hook_style=hook_style)
        if result.get("ok"):
            package = result["data"]
    ...
    if st.button("Generate Podcast Episode", type="primary", use_container_width=True):
        result = generate_podcast_content(podcast_topic, episode_theme, story_tone, target_audience, episode_length, narration_style)
        if result.get("ok"):
            package = result["data"]
            package["active_ai_provider"] = active_provider
            package["active_ai_model"] = active_model
    ...
    provider_options = {"Gemini": "gemini", "OpenAI GPT": "openai"}
    ...
    st.caption("Environment variables: GEMINI_API_KEY, OPENAI_API_KEY, DEFAULT_AI_PROVIDER")
