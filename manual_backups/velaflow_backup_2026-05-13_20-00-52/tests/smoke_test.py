...
def main():
    ...
    assert_true(gemini_fallback_text == "gemini fallback" and openai_fallback_text == "openai fallback", "provider offline fallback failed")
    health = run_healthcheck(type("Settings", (), {"gemini_api_key": "", "openai_api_key": "", "default_ai_provider": "openai", "ffmpeg_path": "ffmpeg"})())
    health_names = [item["name"] for item in health["data"]["checks"]]
    assert_true("Gemini configured" in health_names and "OpenAI configured" in health_names and "Active AI provider" in health_names, "provider healthcheck failed")
