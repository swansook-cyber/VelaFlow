...
def _image_metadata(product_image: Dict[str, Any] | None) -> Dict[str, Any]:
    ...
    return {"path": path, "filename": filename, "attached": bool(path), "note": note}
...
def generate_seller_content(
    ...
    try:
        ...
        script_15 = [
            f"0-3s: เปิดด้วยภาพสินค้าในมือ แล้วพูดว่า '{name} เหมาะกับคนที่อยากได้ {main_point}'",
            f"3-10s: โชว์การใช้งานจริงและชี้จุดเด่นเรื่อง {second_point}",
            "10-15s: ปิดด้วยผลลัพธ์สั้น ๆ และ CTA ให้กดดูรายละเอียด",
        ]
        script_30 = [
            ...
            "ปิดด้วย CTA: 'ใครกำลังหาแนวนี้ ลองกดดูรายละเอียดไว้ก่อนนะ'",
        ]
        script_60 = [
            f"0-5s: เปิดด้วย pain point ของ {audience} ที่ต้องการ {main_point}",
            f"5-15s: แนะนำ {name} พร้อม product beauty shot",
            f"15-30s: สาธิตการใช้งานจริงและเล่าเรื่อง {second_point} แบบ creator",
            f"30-45s: โชว์ดีเทล ขนาด texture หรือ setup จริงที่เกี่ยวกับ {third_point}",
            "45-55s: สรุปว่าเหมาะกับใครและทำไมถึงน่ากดดู",
            "55-60s: ปิดด้วย CTA พร้อม hero shot ของสินค้าในเฟรมแนวตั้ง",
        ]
        ctas = [
        ...
        thumbnail_prompt = (
            ...
            f"creator-style selling visual, emotional but trustworthy, high contrast product focus, no random text"
        )
        broll = [
