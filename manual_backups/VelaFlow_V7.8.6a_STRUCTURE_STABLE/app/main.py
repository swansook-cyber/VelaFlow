...
def _render_song_studio(project: dict[str, Any]) -> None:
    ...
    with st.expander("Song Structure Intelligence", expanded=bool(structure_plan)):
        ...
        if structure_plan:
            ...
            with st.expander("Notes for lyrics / MV Director", expanded=False):
                st.write(structure_plan.get("notes_for_lyrics_generation", ""))
                st.write(structure_plan.get("notes_for_mv_director", ""))
    ...
    if generate_hooks_clicked or regenerate_hooks_clicked:
        ...
        hook_result = generate_hook_candidates_with_provider(
            ...
            idea=_structure_for_song_idea(_direction_for_song_idea(idea, creative_direction), structure_plan, use_structure_plan),
            genre=genre,
            mood=mood,
    ...
    if st.button("Generate Full Lyrics Using Selected Hook", key="song_generate_full_lyrics"):
        ...
        idea_with_hook = (
            f"{_structure_for_song_idea(_direction_for_song_idea(idea, creative_direction), structure_plan, use_structure_plan)}\n\nSelected Hook: {hook.get('hook_text', '')}\n"
