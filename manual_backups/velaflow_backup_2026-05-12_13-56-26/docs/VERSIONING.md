...
V7.8.4 is **Artist Preset Manager**. It adds local/offline preset management for
Song Studio: create, edit, duplicate, import/export, set default, and delete
custom artist presets. Vela Moon is a locked system preset, and the configured
default is stored in `config/artist_presets/default_artist.json`.

## V7.8.5

V7.8.5 is **Navigation & Layout Organization Pass**. It reorganizes the sidebar
into START, SONG, VISUAL, PRODUCTION, INTELLIGENCE, and SYSTEM groups, keeps
internal page keys backward compatible, tightens Song Studio Only filtering, and
standardizes key page headers for daily usage.

## V7.8.5a

V7.8.5a is **Visual Polish Pass**. It adds a global Streamlit style helper for
font rendering, contrast, sidebar readability, KPI cards, button consistency,
table framing, and spacing without changing navigation, provider, render, export,
hook generation, or artist preset logic.

## V7.8.5b

V7.8.5b is **Section Selector Reset Fix**. It fixes sidebar section selection so
valid groups no longer snap back to START when the previous selected page belongs
to another group. Navigation validation now keeps the selected section and moves
the page to that section's first page.

## V7.8.6

V7.8.6 is **Creator Wizard Guided Setup**. It upgrades Creator Wizard from a
basic template loader into a five-step offline creative setup that creates a
structured Creative Direction, saves `creative_direction.json`, and passes topic,
mood, music direction, artist preset, and target platform context into Song
Studio.

## V7.8.6a

V7.8.6a is **Song Structure Intelligence v1**. It adds lightweight producer-style
structure planning before lyric generation: section purpose, energy curve,
emotional arc, hook placement, and English-only arrangement tag suggestions.
Plans are saved as `song_structure_plan.json` and exported alongside Suno files.
