...
from core.suno_export import build_release_package_data
...
def main():
    ...
    assert_true(PAGE_LABELS.get("Smart Clip Factory") == "Clip Factory" and PAGE_LABELS.get("Production Audit") == "Quality Audit", "menu label polish failed")
    ...
    for section in ["SONG METADATA", "LYRICS", "SEO CAPTION", "YOUTUBE DESCRIPTION", "HASHTAGS", "SHORTS HOOKS", "COVER ART PROMPTS"]:
    ...
    assert_true(all(key in release_data["cover_art_prompts"] for key in ["1:1", "16:9", "9:16"]), "cover prompt aspect prompts missing")
