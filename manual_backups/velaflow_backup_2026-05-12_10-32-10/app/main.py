...
from app.presets import DEFAULT_MUSIC_PRESET, get_music_preset, list_music_preset_names, music_preset_prompt
...
def _music_style_with_preset(style_prompt: str, music_preset: dict[str, str]) -> str:
...
def _render_suno_downloads(project_name: str, song: dict[str, Any]) -> None:
    ...
    if not lyrics.strip():
        ...
        return
    export_result = export_suno_files(project_name, song)
    ...
    d1.download_button(
        ...
        file_name="suno_full_package.txt",
        mime="text/plain",
    ...
    if d3.button("Copy Lyrics for Suno", use_container_width=True, key="show_copy_lyrics_for_suno"):
        st.session_state.show_copy_lyrics_for_suno = True
    with d4:
        if st.button("Continue to MV Director", use_container_width=True, key="suno_continue_mv"):
            _continue_to_mv_director()
    with st.expander("Copy Lyrics for Suno", expanded=st.session_state.get("show_copy_lyrics_for_suno", True)):
        st.text_area("Copy-ready lyrics", value=data.get("lyrics_only_text", lyrics), height=260, key="copy_lyrics_for_suno")
    with st.expander("Release Package Copy Blocks", expanded=False):
        c1, c2 = st.columns(2)
        with c1:
            st.text_area("Copy SEO Caption", value=data.get("seo_caption", ""), height=110, key="copy_seo_caption")
            st.text_area("Copy Hashtags", value=data.get("hashtags_text", ""), height=110, key="copy_hashtags")
        with c2:
            st.text_area("Copy YouTube Description", value=data.get("youtube_description", ""), height=160, key="copy_youtube_description")
            st.text_area("Copy Cover Prompts", value=data.get("cover_prompts_text", ""), height=160, key="copy_cover_prompts")
...
def _render_song_studio(project: dict[str, Any]) -> None:
    ...
    with st.expander("Song Structure Intelligence", expanded=bool(structure_plan)):
        ...
        selected_structure_label = st.selectbox("Structure Preset", structure_labels, index=current_structure_index, key="song_structure_preset")
        ...
        if st.button("Generate / Refresh Structure Plan", key="song_generate_structure_plan"):
            context = {
                **creative_direction,
    ...
    with left:
        ...
        genre = st.selectbox("Genre", genre_options, index=genre_index)
        mood_options = ["เศร้า", "คิดถึง", "เหงากลางคืน", "อบอุ่น", "ให้กำลังใจ"]
        mood = st.selectbox("Mood", mood_options, index=1)
    ...
    with right:
        ...
        selected_music_preset_name = st.selectbox("Music Preset", music_preset_names, index=music_preset_index, key="song_music_preset")
    ...
    suno_ready = bool(lyrics_saved and (ROOT / "project_data" / "projects" / safe_name(project.get("title", "project")) / "exports" / "suno_full_package.txt").is_file())
    ...
    generate_hooks_clicked = hook_actions[0].button("Generate Hook Candidates", type="primary", key="song_generate_hooks")
    regenerate_hooks_clicked = hook_actions[1].button("Regenerate Hooks", key="song_regenerate_hooks")
    hook_cache_dir = ROOT / "outputs" / "cache" / "text"
    if hook_cache_dir.exists() and hook_actions[2].button("Clear Hook Cache", key="song_clear_hook_cache"):
    ...
    if generate_hooks_clicked or regenerate_hooks_clicked:
        ...
        hook_result = generate_hook_candidates_with_provider(
            ...
            idea=_music_preset_for_song_idea(_structure_for_song_idea(_direction_for_song_idea(idea, creative_direction), structure_plan, use_structure_plan), selected_music_preset),
            genre=genre,
            mood=mood,
        ...
        hook_style_prompt = _music_style_with_preset(style_override if use_preset else preset.get("default_music_style_prompt", ""), selected_music_preset)
        song.update({
    ...
    if st.button("Generate Full Lyrics Using Selected Hook", key="song_generate_full_lyrics"):
        ...
        idea_with_hook = (
            f"{_music_preset_for_song_idea(_structure_for_song_idea(_direction_for_song_idea(idea, creative_direction), structure_plan, use_structure_plan), selected_music_preset)}\n\nSelected Hook: {hook.get('hook_text', '')}\n"
        ...
        final_style_prompt = _music_style_with_preset(style_override if use_preset else preset.get("default_music_style_prompt", ""), selected_music_preset)
        song_result = _safe(
            "Generate song",
    ...
    if song.get("hook_candidates") or song.get("normalized_song_output") or song.get("complete_lyrics"):
        ...
        with t4:
            ...
            st.json({
                ...
                "suno_full_package": str(ROOT / "project_data" / "projects" / safe_name(project.get("title", title)) / "exports" / "suno_full_package.txt"),
...
if page == "Dashboard":
    ...
    st.caption(
        f"Workflow Mode: {st.session_state.get('workflow_mode', 'Full Pipeline')} | "
