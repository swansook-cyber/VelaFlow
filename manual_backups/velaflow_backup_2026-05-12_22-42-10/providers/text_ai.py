...
def _normalize_song(data: Dict[str, Any], artist_preset: Dict[str, Any] | None = None, force_english_instrument_tags: bool = True) -> Dict[str, Any]:
    ...
    data["hook_candidates"] = hooks
    data["candidate_hooks"] = hooks
    selected_text = data.get("selected_hook_text") or data.get("selected_hook")
    if isinstance(selected_text, dict):
        selected_text = selected_text.get("hook_text") or selected_text.get("text") or ""
    selected_hook = next((hook for hook in hooks if hook.get("hook_text") == selected_text), None) or (select_best_hook(hooks) if hooks else {})
    data["selected_hook"] = selected_hook
    data["selected_hook_text"] = selected_hook.get("hook_text", "") if selected_hook else ""
...
def _offline_song(idea: str, genre: str, mood: str, vocal: str, viral_level: str, artist_preset: Dict[str, Any] | None = None) -> str:
    ...
    return json.dumps({
        ...
        "hook_candidates": [
            {"name": "Hook A", "hook_text": "ยังคิดถึงเธอทุกคืน", "emotional_score": 92, "catchy_score": 88, "tiktok_potential": 86, "reason_th": "สั้น ตรง และร้องตามง่าย", "suggested_usage": "chorus"},
            {"name": "Hook B", "hook_text": "ฝนตกในใจฉัน", "emotional_score": 89, "catchy_score": 84, "tiktok_potential": 82, "reason_th": "เห็นภาพชัด เหมาะกับ MV และแคปชั่น", "suggested_usage": "TikTok clip"},
            {"name": "Hook C", "hook_text": "กลับมาได้ไหม", "emotional_score": 87, "catchy_score": 85, "tiktok_potential": 80, "reason_th": "จำง่ายและใช้เป็น title line ได้", "suggested_usage": "title line"},
        ],
        "candidate_hooks": [
            {"name": "Hook A", "hook_text": "ยังคิดถึงเธอทุกคืน", "emotional_score": 92, "catchy_score": 88, "tiktok_potential": 86, "reason_th": "สั้น ตรง และร้องตามง่าย", "suggested_usage": "chorus"},
            {"name": "Hook B", "hook_text": "ฝนตกในใจฉัน", "emotional_score": 89, "catchy_score": 84, "tiktok_potential": 82, "reason_th": "เห็นภาพชัด เหมาะกับ MV และแคปชั่น", "suggested_usage": "TikTok clip"},
        ...
        "selected_hook": {"name": "Hook A", "hook_text": "ยังคิดถึงเธอทุกคืน", "emotional_score": 92, "catchy_score": 88, "tiktok_potential": 86, "reason_th": "สั้น ตรง และร้องตามง่าย", "suggested_usage": "chorus"},
        "selected_hook_text": "ยังคิดถึงเธอทุกคืน",
...
Output structure:
Part 1: Song Title
Part 2: Target Audience + Hook Strategy
Part 3: Candidate Hooks 3-5 options
Part 4: Instrument Selection
Part 5: Music Style Prompt English only
Part 6: Advanced Settings for Suno
Part 7: Complete Lyrics with Tags
Part 8: TikTok Clip Cut Recommendation

กติกา:
1) ชื่อเพลงไทยสั้น 2-4 พยางค์
2) Music Style Prompt ต้องเป็นภาษาอังกฤษเท่านั้น
3) เนื้อเพลงใช้ Suno tags: [Intro], [Verse 1], [Pre-Chorus], [Chorus], [Post-Chorus], [Verse 2], [Bridge], [Guitar Solo], [Final Chorus], [Outro]
4) เพลงต้องเป็น full-length 3-5 minute arrangement ไม่ใช่เพลงสั้น
5) Candidate Hooks 3-5 แบบ ทุก hook ต้องมี emotional_score, catchy_score, tiktok_potential, reason_th, suggested_usage
...
{{
  ...
  "hook_type": "",
  "hook_candidates": [
    {{"name":"Hook A","hook_text":"","emotional_score":0,"catchy_score":0,"tiktok_potential":0,"reason_th":"","suggested_usage":"chorus"}},
    {{"name":"Hook B","hook_text":"","emotional_score":0,"catchy_score":0,"tiktok_potential":0,"reason_th":"","suggested_usage":"TikTok clip"}},
    {{"name":"Hook C","hook_text":"","emotional_score":0,"catchy_score":0,"tiktok_potential":0,"reason_th":"","suggested_usage":"title line"}}
  ],
  "candidate_hooks": [
    {{"name":"Hook A","hook_text":"","emotional_score":0,"catchy_score":0,"tiktok_potential":0,"reason_th":"","suggested_usage":"chorus"}}
  ],
  "selected_hook": {{"name":"Hook A","hook_text":"","emotional_score":0,"catchy_score":0,"tiktok_potential":0,"reason_th":"","suggested_usage":"chorus"}},
  "selected_hook_text": "",
