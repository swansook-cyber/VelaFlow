...
from core.seller_content import TONE_GUIDES, export_seller_content, generate_seller_content, seller_content_to_text
...
if page == "Dashboard":
    ...
    if st.button("Generate Seller Content", type="primary", use_container_width=True):
        result = generate_seller_content(product_name, product_category, target_audience, key_points, tone_style)
        if result.get("ok"):
            package = result["data"]
    ...
    if seller_package:
        ...
        with t1:
            ...
            for idx, item in enumerate(seller_package.get("short_video_script", []), start=1):
                st.write(f"{idx}. {item}")
