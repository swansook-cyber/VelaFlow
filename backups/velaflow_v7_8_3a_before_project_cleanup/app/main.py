...
def _render_song_studio(project: dict[str, Any]) -> None:
    ...
    if hook_candidates:
        labels = [
            f"{hook.get('name', 'Hook')} - {hook.get('hook_text', '')} | E{hook.get('emotional_score', 0)} C{hook.get('catchy_score', 0)} T{hook.get('tiktok_potential', 0)}"
            for hook in hook_candidates
        ]
        default_label = next((label for label, hook in zip(labels, hook_candidates) if hook.get("hook_text") == selected_hook_text), labels[0])
        selected_label = st.radio("Choose Hook", labels, index=labels.index(default_label), key="song_hook_radio")
        hook = hook_candidates[labels.index(selected_label)]
        hc1, hc2, hc3, hc4 = st.columns(4)
        hc1.metric("Emotional", hook.get("emotional_score", 0))
        hc2.metric("Catchy", hook.get("catchy_score", 0))
        hc3.metric("TikTok", hook.get("tiktok_potential", 0))
        hc4.metric("Usage", hook.get("suggested_usage", "chorus"))
        st.caption(hook.get("reason_th", ""))
        if st.button("Use Selected Hook", key="song_use_selected_hook"):
            ...
            _save_project()
            st.success(f"Selected hook: {hook.get('hook_text', '')}")
            st.rerun()
    else:
...
st.caption(f"{PRODUCT_TAGLINE} by {BRAND_NAME} | V7.8.3a Navigation Flow Fix")
