...
def _render_song_studio(project: dict[str, Any]) -> None:
    ...
    if generate_hooks_clicked or regenerate_hooks_clicked:
        ...
        hook_result = generate_hook_candidates_with_provider(
            ...
            idea=_direction_for_song_idea(idea, creative_direction),
            genre=genre,
            mood=mood,
    ...
    if st.button("Generate Full Lyrics Using Selected Hook", key="song_generate_full_lyrics"):
        ...
        idea_with_hook = (
            f"{_direction_for_song_idea(idea, creative_direction)}\n\nSelected Hook: {hook.get('hook_text', '')}\n"
...
st.caption(f"{PRODUCT_TAGLINE} by {BRAND_NAME} | V7.8.6 Creator Wizard Guided Setup")
...
if page == "Dashboard":
    ...
    with c2:
        ...
        target_platform = st.selectbox("Step 5: Target Platform", TARGET_PLATFORM_OPTIONS, index=5, key="wizard_target_platform_select")
