...
st.caption(f"{PRODUCT_TAGLINE} by {BRAND_NAME} | V7.8.1b Full UI Recovery")
...
with st.sidebar:
    ...
    page = st.radio("Menu", PAGES, index=PAGES.index(current) if current in PAGES else 0, label_visibility="collapsed")
...
if page == "Dashboard":
    ...
    st.dataframe(pd.DataFrame(status.get("data", {}).get("stages", []) or []), use_container_width=True, height=220)
    cols = st.columns(5)
    ...
    tabs = st.tabs(["Scene Score", "Hooks", "Emotional Arc", "Suggestions", "TikTok"])
    with tabs[0]:
