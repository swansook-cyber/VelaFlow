...
def _selected_hook(project: Dict[str, Any]) -> str:
    ...
    for scene in storyboard:
        ...
        if "hook" in text or "chorus" in text:
            return _first_text(scene.get("subtitle_text"), scene.get("lyric_part"), song.get("selected_hook"))
    return _first_text(song.get("selected_hook"), song.get("complete_lyrics", "").splitlines()[0] if song.get("complete_lyrics") else "")
