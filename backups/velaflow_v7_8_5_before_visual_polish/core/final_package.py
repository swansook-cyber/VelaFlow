...
from core.suno_export import build_suno_full_package
...
def build_final_release_package(
    ...
    for sub in ["song", "mv", "clips", "captions", "marketing_package", "thumbnails", "prompts", "exports"]:
    ...
    _write(package_dir / "exports" / "suno_full_package.txt", build_suno_full_package(song, project.get("title", "project")), copied, "exports/suno_full_package.txt")
    ...
    _write(package_dir / "song" / "selected_hook.json", json.dumps(song.get("selected_hook") or {"hook_text": song.get("selected_hook_text", "")}, ensure_ascii=False, indent=2), copied, "song/selected_hook.json")
